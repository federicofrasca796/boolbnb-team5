                ...
                <div class="w-100 d-flex flex-column flex-md-row {{ $apartment->is_aviable === 0 ? 'blur' : '' }}">
                    ...
                    <div class="container_image col-12 col-md-3 mb-2 position-relative">
                        <a class="" href="{{ '/apartments/' . $apartment->slug }}">
                            <img src="{{ asset('storage/' . $apartment->thumbnail) }}" class="h-100 w-100"
                                alt="{{ $apartment->slug }}">
                        </a>
                        @if ($apartment->is_aviable === 0)
                            <!-- modal visibility button trigger -->
                            <a type="button" class="position-absolute top-50 start-50 translate-middle px-4 py-3 text-muted"
                                data-bs-toggle="modal" data-bs-target="#makeVisibile{{ $apartment->slug }}">
                                <i class="fa-solid fa-eye-slash text-white fs-1"></i>
                            </a>
                            <!-- modal visibility -->
                            <div class="modal fade" id="makeVisibile{{ $apartment->slug }}" tabindex="-1"
                                role="dialog" aria-labelledby="modal-{{ $apartment->slug }}Label" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title w-85" id="{{ $apartment->slug }}Label">You're trying to
                                                make the apartment "{{ $apartment->title }}" visible</h5>
                                            <button type="button" class="btn-close align-self-baseline"
                                                data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">

                                            <form action="{{ route('ura.apartments.makeVisibile', $apartment->slug) }}"
                                                method="post">
                                                @csrf
                                                @method('PUT')
                                                <div class="form-group">
                                                    <label for="is_aviable">Do you want to make this apartment
                                                        visible?</label>
                                                    <input required class="ms-3" type="checkbox" name="is_aviable"
                                                        id="is_aviable" value="1">
                                                    <label for="">YES</label>
                                                </div>
                    ...
                    <div class="col-12 col-md-7">
                        <div class="card-body d-flex flex-column h-100 justify-content-between">
                            ...
                            <small class="text-muted">Created on {{ $apartment->created_at->toDateString() }} - Last
                                update {{ $apartment->updated_at->toDateString() }}</small>
                        </div>
